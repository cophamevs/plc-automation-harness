...
<!-- Workflow for opening an existing TIA Portal project, exploring its structure,
     exporting blocks, making changes, recompiling, and saving. -->

## Frontmatter
- **Tags**: modify, edit, existing, open, export, import, reimport, save
...
If `ErrorCount > 0`, follow the **[debug-compile-errors.md](debug-compile-errors.md)** workflow, then return here.
...
| `CompileSoftware` returns errors after modification | Change introduced a type mismatch, missing symbol, or structural error | Follow **debug-compile-errors.md**; the quick-reference table covers the most common patterns |
